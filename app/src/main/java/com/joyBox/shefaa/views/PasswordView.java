package com.joyBox.shefaa.views;
...
public class PasswordView extends AppCompatEditText {
    ...
    */
    ...
    private int eyeTint = Color.WHITE;
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            eyeTint = typedArray.getColor(R.styleable.PasswordView_eyeTint, Color.WHITE);
            typedArray.recycle();
        }
    ...
    private void setup() {
        ...
        if (Build.VERSION.SDK_INT >= 17)
            setCompoundDrawablesRelativeWithIntrinsicBounds(null, drawables[1], drawable, drawables[3]);
        else
    ...
    public boolean onTouchEvent(MotionEvent event) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= 17) {
                if (this.getLayoutDirection() == LAYOUT_DIRECTION_LTR) {
                    if (event.getAction() == MotionEvent.ACTION_UP
                            && event.getX() >= (getWidth() - getCompoundDrawablesRelative()[2].getBounds().width() - getPaddingRight())
                    ...
                    if (event.getAction() == MotionEvent.ACTION_UP
                            ...
                            && event.getX() <= (getCompoundDrawablesRelative()[2].getBounds().width() + getPaddingRight())) {
                        visible = !visible;
                        setup();
                        invalidate();
                        this.cursorPosition = getSelectionStart();
